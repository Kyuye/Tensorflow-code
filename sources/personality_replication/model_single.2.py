...
class WasserstienGAN(object):
    ...
    def get_feed(self, pl_train, pl_label, minibatch):
        seq_vec = []
        for seq in minibatch[0]:
        ...
        return {pl_train: seq_vec, pl_label: indices}

    # def get_batch(self, data, iterator, minibatch_size, fullbatch_size):
    #     train_batch = data[(iterator*minibatch_size)%fullbatch_size:((iterator+1)*minibatch_size)%fullbatch_size]
    #     return train_batch['content'], train_batch['Sentiment']
    ...
    def _create_network(self, optimizer="Adam", learning_rate=2e-4, optimizer_param=0.9):
        ...
        # real_pairs = self.build_generated_pair_set(self.train_batch)
