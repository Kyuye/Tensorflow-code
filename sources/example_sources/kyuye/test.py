...
# global variables set in tensorflow 
...
# constructor
    ...
    def _gen_sim_data(cls) :
        ...
        cls.fbatch_input = tf.reshape(cls.ts_y[:-1], sp_batch)
        cls.fbatch_label = tf.reshape(cls.ts_y[1:],sp_batch)
    ...
    def _build_batch(cls) :
        ...
        cls.mb_train, cls.mb_label = tf.train.batch(cls.batch_set, CONST.batch_size, enqueue_many=True)

    @classmethod # buidling rnn model
    def _build_model(cls) :

        cls.rnn_cell = tf.contrib.rnn.BasicRNNCell(CONST.state_size)
        # MAKING RNN tf.contrib.rnn.static_rnn(cell, inputs), return : output, state(= _))
        cls.output, cls.state = tf.contrib.rnn.static_rnn(cls.rnn_cell, tf.unstack(cls.mb_train, axis=1), dtype=tf.float32)
        # Making weight = tf.Variable(tf.truncated_normal([size]))
        cls.output_w = tf.Variable(tf.truncated_normal([CONST.time_step, CONST.state_size, CONST.vec_size]))
        cls.output_b = tf.Variable(tf.zeros([CONST.vec_size]))

        cls.pred = tf.matmul(cls.output, tf.unstack(cls.output_w,axis=0))+ cls.output_b

        # print("the shpae of output_w:",cls.sess.run(tf.shape(cls.output_w)))
        # print("the shape of output_b:",cls.sess.run(tf.shape(cls.output_b)))
    ...
    def _build_train(cls) :
        ...
        for i in range(CONST.time_step) : 
            # loss on each step consist of list(from unstack function)shape   
            cls.loss += tf.losses.mean_squared_error(tf.unstack(cls.mb_label,axis=1)[i],cls.pred[i])
            # print(cls.sess.run(cls.output))
...
def main(_) :
    ...
    rnn = rnn_example() 
    # running the class rnn_examplels.pr
    rnn.run()
    toc = time.clock()
