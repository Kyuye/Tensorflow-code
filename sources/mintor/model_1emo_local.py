...
class WassersteinGAN(object):
    ...
    def create_network(self, optimizer="Adam", learning_rate=2e-4, optimizer_param=0.9):
        ...
        for g in range(FLAGS.gpu_num):
            with tf.device("/gpu:%d"%g):
                ...
                if g == 0:
                    self.gen_grads = self.optim.compute_gradients(
                        self.gen_loss, self.gen_variables)
                    self.disc_grads = self.optim.compute_gradients(
                        self.disc_loss, self.disc_variables)
                else:
                    self.gen_grads += self.optim.compute_gradients(
                        self.gen_loss, self.gen_variables)
                    self.disc_grads += self.optim.compute_gradients(
    ...
    def train_model(self, max_iterations):
        ...
        clip_discriminator_var_op = [
            var.assign(tf.clip_by_value(var, self.clip_values[0], self.clip_values[1])) 
            for var in self.disc_variables]
        ...
        for epoch in range(max_iterations): 
            for itr in range(1, train_step):
                ...
                feed_dict = {}
                for g in range(FLAGS.gpu_num):
                    start = g*FLAGS.batch_size
                    end = start+FLAGS.batch_size
                    feed_dict[self.train_batch[g]] = train_data[start:end]
                    feed_dict[self.label_indices[g]] = indices[start:end]
            ...
            print("Step: %d, generator loss: %g, discriminator_loss: %g" % (
                itr+itr*FLAGS.epoch, g_loss_val, d_loss_val))
